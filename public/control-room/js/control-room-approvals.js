      ...
      $('#appr-queue').innerHTML = items.map((a) => `
        <article class="ticket">
          <div>
            ...
            <div class="meta"><span class="mono">${esc(a.tool)}</span><span>${ago(new Date(a.createdAt).toISOString())}</span><span>workspace: <span class="mono">${esc(a.workspaceId)}</span></span>${a.origin && a.origin.kind === 'external-agent' ? `<span class="chip sq">source: external agent <span class="mono">${esc(a.origin.ref || 'unknown')}</span></span>` : ''}</div>
