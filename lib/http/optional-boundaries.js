...
const { createEmergencyStopBoundary } = require('./emergency-stop-routes');
...
function createOptionalRouteBoundaries(options = {}) {
  ...
  // The emergency stop shares the memory approval credentials and response
  // helpers: both are operator decisions behind the same operator token.
  const emergencyStop = createEmergencyStopBoundary({
    ...(options.emergencyStop || {}),
    operatorToken: memoryApprovalOptions.operatorToken,
    parseJsonRequest: (...args) => memoryApprovalOptions.getParseJsonRequest()(...args),
    writeJson: (...args) => memoryApprovalOptions.getWriteJson()(...args),
  });
  ...
  return Object.freeze({
    ...
    async route(req, res, reqUrl) {
      ...
      if (await emergencyStop.route(req, res, reqUrl)) return true;
