...
/**
 * Best-effort audit write for this module's verdict events (#2345).
 *
 * This reaches the trail only through Graph's public surface. Kernel's
 * `_appendAuditEvent` stays the Kernel-internal chokepoint and is never
 * renamed, aliased, or called from here (ADR-012 pins its name and its
 * call sites). The three observable behaviors match: no graph, no write;
 * provenance wrapped the same way; never throws.
 */
function appendReviewAuditEvent(kernel, event, provenance, workspaceId) {
  ...
  try {
    return graph.appendAuditEvent(event, provenance ? { provenance, workspaceId } : { workspaceId });
  } catch (error) {
    console.error('[hypothesis-review] Audit log error:', error.message);
    return null;
  }
}
