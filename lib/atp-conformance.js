...
// One row per ATP object type (#2182): adding a type is a row here, not a new
// case. No prototype, so an inherited key never resolves to a validator.
const ATP_VALIDATORS = Object.freeze(Object.assign(Object.create(null), {
  [ATP_OBJECT_TYPES.provenanceRecord]: validateProvenanceRecord,
  [ATP_OBJECT_TYPES.auditEvent]: validateAuditEvent,
  [ATP_OBJECT_TYPES.candidateClaim]: validateCandidateClaim,
  [ATP_OBJECT_TYPES.conflictResult]: validateConflictResult,
  [ATP_OBJECT_TYPES.verificationResult]: validateVerificationResult,
  [ATP_OBJECT_TYPES.trustReceipt]: validateTrustReceipt,
  [ATP_OBJECT_TYPES.causalChain]: validateCausalChain,
  [ATP_OBJECT_TYPES.simulationResult]: validateSimulationResult,
  [ATP_OBJECT_TYPES.error]: validateErrorObject,
}));

function validateATPObject(type, object, opts = {}) {
  const normalizedType = String(type || '').trim();
  if (Object.hasOwn(ATP_VALIDATORS, normalizedType)) return ATP_VALIDATORS[normalizedType](object, opts);
