 ...
 * #2393 has not shipped in this branch (no `lib/experience/registry.js` or
 * equivalent exists yet), so `boundProcedureVersion` is treated here as an
 * opaque, caller-supplied string identifying one procedure version — a
 * `procedure.hash` from `lib/experience/compiler.js`, for example. Nothing
 * in this module interprets it beyond equality comparison.
 *
 ...
 * of `createLearningPool()` in ./learning.js: the registry is caller-held,
 * durable storage is a separate concern left to whoever wires this in.
 *
 * ## Trust states
 *
 * `insufficient-data | probationary | trusted | demoted` (see the design
 * comment on #2383 for the full rationale). A caller must read the explicit
 * state, never a boolean — `insufficient-data` and `demoted` must never be
 * conflatable.
 *
 * ## State derivation
 *
 * On every recorded run the state is *recomputed from evidence*, not
 * incrementally patched, which is what makes "one negative drops trusted to
 * probationary within the same event" fall out of the formula rather than
 * needing a special case: `trusted` requires zero negatives in the trailing
 * window, so a single negative removes eligibility the instant it is
 * recorded. Two explicit actions sit outside the formula on purpose:
 *
 * - **Procedure rebind** forces `probationary` regardless of prior state,
 *   even though the new procedure version has zero evidence (which the
 *   formula alone would read as `insufficient-data`). This is why the
 *   registry entry carries a `floor`: once a capability has been rebound at
 *   least once, "no evidence yet for the current version" no longer reads
 *   as "unknown," it reads as "not yet reproven." A capability that has
 *   never been rebound (freshly created, including a composed one) has no
 *   floor and starts at `insufficient-data`, per acceptance test 7.
 * - **Operator/policy block** forces `demoted` unconditionally and stays
 *   there — immune to the evidence formula — until explicitly cleared.
 *
 * Evidence for a superseded procedure version is never deleted, only
 * excluded from the active window (it stays in `history`/the raw event log,
 * version-labeled).
 *
 * `promoteCanaryCandidate`/`rollbackToPriorVersion`/`getEvidenceForVersion`/
 * `proposeDriftRollback` (#2397) extend this same ladder via `rebindProcedure`
 * above; code lives in `./capability-trust-canary-extension.js` for
 * check:file-size budget reasons (see that file's doc for the rationale).
 */
