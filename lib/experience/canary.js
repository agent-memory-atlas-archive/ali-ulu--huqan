 ...
 *
 * Canary is a new state on the Capability Trust record, sitting between a
 * freshly-compiled candidate procedure and eligibility for #2394's existing
 * promotion evaluation. This module owns three things, pure and hermetic
 * (no I/O, no storage, no model call, no journal/registry import):
 *
 * - `totalCost`: execution cost plus independent verification cost plus,
 *   only while a candidate is on trial, canary overhead
 *   (`executionCost + verificationCost + (candidateOnly ? canaryOverheadCost : 0)`).
 *   A candidate is never compared on execution time alone while its
 *   verification is more expensive — a real operator pays both.
 * - Same-distribution sampling: candidate and baseline canary runs are
 *   interleaved on live-matching requests. A comparison between a
 *   candidate's canary sample and baseline's comparison window is refused
 *   (`ok: false`), never silently scored, when the two samples do not
 *   overlap on request shape. Overlap is checked by reusing
 *   `router.js`'s structural-match vocabulary directly (`structuralMatch`),
 *   not a re-derived notion of "similar enough."
 * - Bounded pass/fail: a candidate's `totalCost` AND `negative_example`
 *   rate over its canary sample must both be at least as good as
 *   baseline's comparison-window values, and a tie on both does not pass
 *   — the candidate must clear baseline, not merely match it, mirroring
 *   #2394's own "falls fast, climbs slow" asymmetry. The trial itself is
 *   capped at `DEFAULT_CANARY_MAX_RUNS` runs or `DEFAULT_CANARY_MAX_DAYS`
 *   days, whichever comes first; if the cap is reached without enough
 *   samples to decide, the trial fails closed rather than passing on
 *   insufficient evidence.
 *
 * ## What this module does NOT do
 *
 * It never calls `capability-trust.js`'s registry itself (no import of
 * that module — the dependency runs the other way: `capability-trust.js`'s
 * extension imports this module to decide whether a rebind/promotion may
 * proceed). It never routes a real request; `shouldRouteToCandidate()` is
 * a pure decision function the caller's router/dispatcher consults per
 * request. It never auto-promotes or auto-reverts: `resolveAdmission()`
 * only reports whether an admission record already exists, mirroring
 * `lib/human-approval-toggle.js`'s pattern of never silently producing
 * approval — a promotion or rollback with no explicit approval and no
 * active toggle simply is not admitted.
 */
