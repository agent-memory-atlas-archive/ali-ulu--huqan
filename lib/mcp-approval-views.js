...
/**
 * Normalized origin label for the approval screen (#2592, confused deputy).
 *
 * Read-only derivation: a caller that declares external-agent origin
 * (provenance.sourceType 'agent') is shown as such, with the declared actor
 * or sourceRef as the ref. Everything else is 'unknown' -- never 'local',
 * because an MCP call carries no caller identity and claiming locality
 * would be the exact lie this label exists to prevent.
 */
function deriveApprovalOrigin(approval, args) {
  const provenance = (approval.context && typeof approval.context.provenance === 'object' && approval.context.provenance)
    || (args && typeof args.provenance === 'object' && args.provenance)
    || {};
  if (String(provenance.sourceType || '').toLowerCase() === 'agent') {
    const ref = String(provenance.actor || provenance.sourceRef || '').slice(0, 128).trim();
    return Object.freeze({ kind: 'external-agent', ref: ref || 'unknown' });
  }
  return Object.freeze({ kind: 'unknown', ref: null });
}
...
function projectApprovalRecord(record) {
  ...
  return redactSecretValues({
    ...
    origin: deriveApprovalOrigin(approval, args),
