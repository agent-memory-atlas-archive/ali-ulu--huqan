...
function journalReaderOf(source) {
  if (typeof source?.readJsonJournal === 'function') return source.readJsonJournal.bind(source);
  return undefined;
}
...
function receiptSchemaFamily(source, receiptId, workspaceId, explicitFamily) {
  try {
    ...
    if (!family && (source?._db || typeof source?.readJsonJournal === 'function')) {
      family = trimText(getReceiptFamilyById(source, receiptId, workspaceId,
        { readJsonJournal: journalReaderOf(source) }));
    }
...
function validationStamp(options, source, receiptId, workspaceId) {
  ...
  try {
    const state = typeof source?.getReceiptStamp === 'function'
      ...
      : source && (source._db || typeof source?.readJsonJournal === 'function')
        ? getReceiptStamp(source, workspaceId, schemaFamily, { readJsonJournal: journalReaderOf(source) })
        : null;
