...
const { summarizeRunImpact } = require('./run-impact');
...
function finalizeAgentRun({ storage, state, goalMemory, saveCheckpoint }) {
  // The run's cumulative firewall risk, recorded with the run and not enforced
  // (#2505). saveRun persists the whole state, so it needs no new column.
  state.impact = summarizeRunImpact(state.steps);
