...
const { enforceAgentActionStep } = require('./agent-action-firewall');
...
function executeInternalTool(runtime, step, state, opts) {
  const { kernel, dream } = runtime;
  switch (step.tool) {
    case 'learn': return kernel.learn(step.input, opts.learnOpts || {});
    case 'ask': return kernel.ask(step.input, opts.askOpts || {});
    case 'verify': return kernel.verify(step.input, opts.verifyOpts || {});
    case 'reason': return kernel.reason(stripQuestionMarks(step.input || state.goal), opts.reasonOpts || {});
    case 'compare': {
      const text = String(step.input || state.goal);
      const parts = text.split('|').map(value => value.trim()).filter(Boolean);
      return parts.length >= 2
        ? kernel.compare(parts[0], parts[1], opts.compareOpts || {})
        : kernel.compare(firstWords(text, 2), firstWords(text.split(/\s+/).slice(2).join(' '), 2), opts.compareOpts || {});
    }
    case 'dream': return dream ? dream.dream(opts.dreamOpts || {}) : kernel.dream(opts.dreamOpts || {});
    default: return unsupportedToolResult(step, runtime.allowedTools);
  }
}
...
function executeAgentStep({ step, state, opts = {}, runtime }) {
  ...
  emit('afterTask', { step: stepReport, state, opts });
  return stepReport;
}
