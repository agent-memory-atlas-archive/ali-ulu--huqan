...
// One row per finding kind: its default severity (#2274). FINDING_KINDS is
// derived from this table, so adding a kind is one row and a kind cannot be
// added without a default. No prototype: an inherited key never matches.
const FINDING_DEFAULT_SEVERITY_BY_KIND = Object.freeze(Object.assign(Object.create(null), {
  bug: 'medium',
  security: 'high',
  flaky_test: 'low',
  stale_docs: 'info',
...
const FINDING_KINDS = Object.freeze(Object.keys(FINDING_DEFAULT_SEVERITY_BY_KIND));
...
function classifyFindingSeverity(input = {}) {
  ...
  return typeof input.kind === 'string' && Object.hasOwn(FINDING_DEFAULT_SEVERITY_BY_KIND, input.kind)
    ? FINDING_DEFAULT_SEVERITY_BY_KIND[input.kind]
    : 'medium';
}
...
function validateFinding(finding) {
  ...
  if (normalized.evidence) normalized.evidence.forEach((item, index) => validateEvidenceItem(item, index, errors));
...
module.exports = {
  FINDING_DEFAULT_SEVERITY_BY_KIND,
  FINDING_KINDS,
  FINDING_SEVERITIES,
  FINDING_STATUSES,
  EVIDENCE_TYPES,
