...
function buildApprovalAdmissionOptions(approval, args = {}) {
  ...
  const storedProvenance = context.provenance && typeof context.provenance === 'object'
    ? context.provenance
    : {};
  // #2592: a caller that declares external-agent origin (sourceType 'agent')
  // must bring its own provenance. When both ids are absent this module would
  // otherwise synthesize them, making an unproven instruction look proven.
  const declaredAgentOrigin = String(storedProvenance.sourceType || '').toLowerCase() === 'agent';
  const agentOriginUnproven = declaredAgentOrigin && !storedProvenance.provenanceId && !storedProvenance.sourceRef;
