...
// #2208: SDK command aliases for plugin capabilities. Data, not control flow:
// a new alias is one row. Keys are stored after normalizeCommandName; an
// unknown command resolves to null. A Map has no inherited keys.
const SDK_COMMAND_TO_CAPABILITY = new Map(Object.entries({
  mri: 'ideaMri',
  ideamri: 'ideaMri',
  devil: 'devilAdvocate',
...
function resolveCapabilityName(command) {
  return SDK_COMMAND_TO_CAPABILITY.get(normalizeCommandName(command)) ?? null;
}
