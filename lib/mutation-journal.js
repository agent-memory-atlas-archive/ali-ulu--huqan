...
const { readMutationReceiptFromJsonJournal } = require('./graph-mutation-receipt-read');
...
function readCommittedMutationResult(graph, operationId) {
  ...
  const journal = graph.readJsonJournal();
  const row = journal.operations[id];
  if (!row || row.status !== 'completed') return null;
  return { operationId: id, status: row.status, result: row.result, committedAt: row.committedAt, receipt: readMutationReceiptFromJsonJournal(journal, id) };
}
...
function readCommittedMutationResultsByPrefix(graph, prefix) {
  ...
  return Object.entries(journal.operations)
    ...
    .map(([operationId, row]) => ({ operationId, status: row.status, result: row.result, committedAt: row.committedAt, receipt: readMutationReceiptFromJsonJournal(journal, operationId) }));
}
