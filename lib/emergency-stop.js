...
function createEmergencyStop({ directory, environment = process.env, now = () => new Date().toISOString() } = {}) {
  ...
  function appendReceipt(action, record, actor, reason) {
    const payload = {
      ...
      actor: text(actor), reason: text(reason), createdAt: now(),
    };
  ...
  function appendLedgerEntry({ action, scope, workspaceId, agentId, actor, reason }) {
    ...
    const payload = {
      ...
      agentId: agentId ?? null, actor: text(actor), reason: text(reason), createdAt: now(),
    };
  ...
  function stop({ scope, workspaceId, agentId, reason, actor } = {}) {
    ...
    const record = {
      ...
      reason: text(reason), actor: text(actor), stoppedAt: now(),
    };
    try {
      fs.writeFileSync(recordPath(target), JSON.stringify(record), { encoding: 'utf8', flag: 'wx' });
    } catch (error) {
    ...
    return Object.freeze({ ok: true, created: true, record, receipt: appendReceipt('stop', record, actor, reason), ledgerEntry, integrityViolation: !integrity.ok });
  }
  ...
  function lift({ scope, workspaceId, agentId, reason, actor } = {}) {
    ...
    if (target.scope === SCOPES.WORKSPACE) {
      const entries = readLedgerEntries(root).filter((entry) => !entry.__unreadable && !entry.__corrupt);
      const state = replayLedgerState(entries, EMERGENCY_STOP_SCHEMA_VERSION).get(scopeKey(target.scope, target.workspaceId, target.agentId));
      if (!state || !state.stopped) {
        return Object.freeze({ ok: true, lifted: false, receipt: null, ledgerEntry: null });
      }
      if (state.lastStopActor && text(actor) === state.lastStopActor) {
        return Object.freeze({ ok: false, lifted: false, reason: QUORUM_REASON, details: { scope: target.scope, workspaceId: target.workspaceId, actor: text(actor), stopActor: state.lastStopActor } });
      }
    }
    try {
    ...
    return Object.freeze({ ok: true, lifted: true, receipt: appendReceipt('lift', target, actor, reason), ledgerEntry });
  }
  ...
  return Object.freeze({ directory: root, stop, lift, check, verifyIntegrity, _readLedgerEntries: () => readLedgerEntries(root), _verifyLedgerEntries: (entries) => verifyLedgerEntries(entries) });
}
